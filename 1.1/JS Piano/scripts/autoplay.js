...
function Autoplay() {
    ...
    this.generateAutoplayTrack = function() {
        ...
        if(sheetDisplay.openedSheet == "default") {
            ...
            this.track.innerHTML += '<span id="'+(this.trackID+"0")+'" class="'+this.trackID+'">'+"-START-"+'</span><br>';
            for(i = 1; i < 8; i++) {
                this.track.innerHTML += '<span id="'+(this.trackID+i)+'" class="'+this.trackID+'"></span><br>';
            }
            
            var sheet = presetSheets[sheetIndex][SheetProperties.SHEET];
            for(j = 0; j < sheet.length; j++) {
                sheet[j] = sheet[j].replace(/&nbsp;/g, ' ');
            }
            
            for(j = 0; j < sheet.length; j++) {
                if(sheet[j][0] != "~") {
                    this.track.innerHTML += '<span id="'+(this.trackID+i)+'" class="'+this.trackID+'">'+sheet[j]+'</span><br>';
                    i++;
                }
            }
            ...
            this.track.innerHTML += '<span id="'+(this.trackID+i)+'" class="'+this.trackID+'">'+"-END-"+'</span><br>';
            i++;
